...
require_once("../connexion/SQLconnect.php");
  ...
  if ($langueJeu == "multi") {
    if (!($req = $con->prepare("SELECT * FROM FriseChrono WHERE lieu = ? AND etat = 'publier'"))) {
    ...
    if (!($req = $con->prepare("SELECT * FROM FriseChrono WHERE lieu = ? AND langue_defi = ? AND etat = 'publier'"))) {
      echo "Echec de la pr̩paration : (" . $con->errno . ") " . $con->error;
    }
