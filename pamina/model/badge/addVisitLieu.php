<?php
	...
	// update nb_lieu_visite
	require_once("../../connexion/SQLconnect.php");

	$id = $_POST['idUser'];
	$ed = strtolower($_POST['ed']);

	if (!($req = $con->prepare("UPDATE Utilisateur SET nb_lieu_visite = nb_lieu_visite + 1 WHERE id=?"))) {
	...
	// Par ED
	$sql = "UPDATE Utilisateur SET nb_lieu_visite_".$ed." = nb_lieu_visite_".$ed." + 1 WHERE id=?";
